...
Get-PSDrive
```
Результат покаже не лише фізичні диски, а й псевдо-диски:
...
```
**Ключ `-Recurse` (Рекурсивно):** Змушує командлет працювати не лише із зазначеним елементом, а й з усім його вмістом.
...
> *   **У будь-якому іншому місці:** Використовуйте символ зворотного апострофа (backtick) `` ` `` в кінці рядка, а потім натисніть `Enter`. Цей символ говорить PowerShell: "Команда продовжиться на наступному рядку".
...
**Знайти всі тимчасові файли (.tmp) та файли бекапів (.bak) для очищення:**
...
Дуже багато файлів/папок — -Recurse може рекурсивно заходити в десятки тисяч елементів.
...
> **Ключ `-WhatIf` (Що якби?):** Ваш найкращий друг. Він **не виконує** команду, а лише виводить у консоль повідомлення про те, **що б сталося**.
...
# Лише переконавшись, що все вірно, прибираємо -WhatIf і ВИКОНУЄМО команду
...
> **Dot Sourcing (`. .\script.ps1`):** Ця спеціальна команда виконує скрипт у *поточному* контексті, роблячи всі його функції та змінні доступними у вашій консолі.
...
> **Що таке профіль PowerShell?** Це спеціальний скрипт `.ps1`, який PowerShell автоматично запускає щоразу при старті. Все, що ви помістите в цей файл — псевдоніми, змінні та функції — буде доступно в кожній вашій сесії за замовчуванням.
...
function Find-DuplicateFiles {
    ...
    Get-ChildItem $Path -File -Recurse -ErrorAction SilentlyContinue |
        ...
        ForEach-Object {
            # ЦЕ ВИПРАВЛЕНИЙ РЯДОК:
            # Всередині оператора $() змінні не екрануються.
...
>Що таке профіль PowerShell? Це спеціальний скрипт .ps1, який PowerShell автоматично запускає щоразу при старті. Все, що ви помістите в цей файл — псевдоніми, змінні та функції — буде доступно в кожній вашій сесії за замовчуванням.
1.  **Знаходимо шлях до файлу профілю.** PowerShell зберігає його у змінній `$PROFILE`.
    ```powershell
    $PROFILE
    ```
...
| `Set-Location` | Переміститися до іншої директорії. | `Set-Location C:\Temp` |
...
*   **Робота зі шляхами (Path)**
    ...
    *   **`Resolve-Path`**: Перетворює відносний шлях (наприклад, `.` або `..\files`) на повний, абсолютний.
...
*   **Просунута навігація (Location Stack)**
    *   **`Push-Location`**: "Запам'ятовує" поточну директорію та переходить до нової.
...
Потрібно **змінити атрибут файлу**, наприклад, зробити його «лише для читання»? Використовуйте `Set-ItemProperty`.
