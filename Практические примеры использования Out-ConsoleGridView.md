...
### Анализ: Почему вы видите только сетевой адаптер

1.  
2.  **Что на вашем скриншоте?** Вы видите события, где Диспетчер управления службами отправил команду **драйверу вашего Wi-Fi адаптера** (`Intel(R) Dual Band Wireless-AC 3165`).
3.  **Главная проблема:** У таких "драйверных" событий **другая структура данных**. В них есть только одно свойство (`Properties[0]`), которое содержит имя драйвера. Свойства для имени службы и ее состояния (`Properties[1]`) в них **просто отсутствуют**. Поэтому колонка `State` у вас пустая.

### Как это исправить: Делаем скрипт умнее
...
#### Пример 6: Анализ запуска и остановки СЛУЖБ
