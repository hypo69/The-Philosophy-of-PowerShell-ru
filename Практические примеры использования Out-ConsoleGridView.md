...
Великолепно! Это именно те данные, которые были нужны. Теперь картина абсолютно ясна.

Скриншот **Команды №2** — самый важный. Давайте разберем его по частям.

### Финальный диагноз (подтвержденный данными)

1.  **`$event.Properties | Get-Member`**:
    *   **`TypeName: System.Diagnostics.Eventing.Reader.EventProperty`**: Это говорит нам, что `$event.Properties` — это **не массив**, как мы ожидали, а **один единственный объект** типа `EventProperty`.
    *   **У него нет свойства `Count`**: Вот почему наша проверка `Where-Object { $_.Properties.Count -ge 2 }` не работала. Мы пытались получить свойство `Count` у объекта, у которого его нет.
    *   **У него есть свойство `Value`**: Это единственное, что нас интересует — значение этого свойства.

2.  **`$event.Properties`**:
    *   Когда вы выводите этот объект, PowerShell по умолчанию показывает его свойство `Value`.
    *   И мы видим, что это значение — **многострочный текст**, а не массив отдельных элементов. Он содержит имя устройства (`\Device\NDMP2...`) и какие-то бинарные данные (`{0, 0, 8, 0...}`).

**Вывод:**
Наше предположение о том, что `$event.Properties` — это массив, было неверным. PowerShell отдает нам коллекцию объектов `EventProperty`, и для доступа к их значениям нам нужно обращаться к свойству `.Value`.

### Исправление: Как работать с этим правильно

Раз мы не можем фильтровать по количеству свойств, нам придется вернуться к идее фильтрации по **провайдеру (источнику)**. Это самый надежный способ, и теперь мы понимаем, почему предыдущие попытки были неудачными.

Давайте вернемся к **финальной версии Примера 6** из моего предыдущего ответа. Я на 99.9% уверен, что она решит проблему, потому что она основана на правильной диагностике и использует самый точный метод фильтрации.

Я повторю его здесь для удобства. Пожалуйста, попробуйте выполнить именно этот код.

---

#### Пример 6:  Анализ системных событий: запуск и остановка служб

Этот скрипт использует фильтрацию по **провайдеру (источнику)** события, чтобы гарантированно получить только данные о запуске и остановке стандартных служб Windows, игнорируя события от драйверов, которые создавали "шум".

```powershell
# Ищем 500 последних событий от Диспетчера управления службами (Service Control Manager)
# Указываем ProviderName, чтобы отсечь события от других источников с таким же ID
...
# Если события найдены, формируем таблицу и выводим в интерактивный интерфейс
if ($serviceEvents) {
    # Теперь мы уверены, что у этих событий есть нужные нам Properties[0] и Properties[1]
    $serviceEvents | 
    ...
    Write-Warning "Не найдено событий о запуске/остановке служб от 'Service Control Manager'."
}
```

**Почему этот код должен сработать:**
*   Он больше не делает предположений о структуре `Properties`.
*   Он напрямую говорит системе: "Дай мне события с ID 7036, но **только от провайдера `Service Control Manager`**".
*   Все события от этого провайдера **гарантированно** имеют одинаковую структуру с двумя свойствами, которые нам нужны: имя службы и ее состояние.

Этот процесс отладки — отличный пример того, как `Get-Member` и `Format-List *` помогают нам исследовать неизвестные объекты и понимать, как с ними правильно работать. Это и есть суть философии PowerShell.
